...
int main (int argc, char *argv[]) {
	int c=0;
	int optc ,  subnet_addr = 0 , port_addr = 0, bind2Ip = 0;
  int verbose = 0 ;  
	int artnet_sd ;
	char *ip_addr = NULL, *bCast = NULL ;
  artnet_start_params anp ;  

	atexit(cleanup);

	dmx = (dmx_t *)malloc(MAXCHANNELS) ;
	...
	// parse options 
	while ((optc = getopt (argc, argv, "vbc:s:p:a:")) != EOF) {
		switch (optc) {
	        case 'a':
  ...
  anp.bind2Ip = bind2Ip ;
  if (!bCast)
    strcpy(anp.cBcastAddr,"") ;
  else
    strcpy(anp.cBcastAddr, bCast) ;
  
  if ( (node = artnet_new_mod(ip_addr, anp) ) == NULL ) {
  // ### LH end
		printf("artnet_new_mod failed %s\n" , artnet_strerror() ) ;
		return 0 ;
	}
...
#ifdef WIN32  
  // soft function-key labels
  // skl_init() must be called BEFORE initscr() !
  slk_init(55) ;  
#endif
...
#ifdef WIN32  
  PDC_set_title("ArtNet-DMXMonitor") ;
  PDC_set_blink(false) ;  
  ...
  noecho();
  curs_set ( 0 ) ;
  // without this refresh following windows 
  // (for example wStatus will) not be shown
  refresh() ;  
#endif
	savetty();
	start_color();
  ...
  keypad ( stdscr, TRUE );
	...
	while (c!='q') {
		...
		FD_ZERO(&rd_fds);
		FD_SET(0, &rd_fds);
		FD_SET((unsigned)artnet_sd, &rd_fds) ;
