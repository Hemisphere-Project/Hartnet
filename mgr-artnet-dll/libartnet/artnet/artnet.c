...
// ### LH - additional parameters for new features
artnet_node artnet_new_mod(const char *ip, artnet_start_params anp)
{
  node  n;
  int   i;
  struct in_addr bCastForce ;  
  ...
  if (!anp.cBcastAddr || !strlen(anp.cBcastAddr))
    strcpy(anp.cBcastAddr, "0.0.0.0") ;
#ifndef WIN32  // ### LH - port to win32  
  if (inet_aton(anp.cBcastAddr, &bCastForce) == 0) {
#else
  if ((bCastForce.s_addr = inet_addr(anp.cBcastAddr)) == INADDR_NONE) {
#endif    
    artnet_error("Warning: error converting %s ? -> INADDR_NONE (255.255.255.255)", anp.cBcastAddr);
  ...
  n->state.verbose  = anp.verbose;
  n->state.bind2Ip  = anp.bind2Ip ;   							// ### LH - parameters for additional features
	n->state.reuse		= anp.reuse ;										// ### LH - reuse ip port ?
  //n->state.allowAllIf = anp.allowAllIf ;            // ### LH - 23.10.2008
  n->state.bCastForce.s_addr = bCastForce.s_addr ;  // ### LH - force another broadcast address
                                                    // if != INADDR_ANY (0.0.0.0)  ...
...
artnet_node artnet_new(const char *ip, int verbose) {
  ...
  n->state.bind2Ip  = 0 ;
	n->state.reuse		= 0 ;
  //n->state.allowAllIf = 0 ;
  n->state.bCastForce.s_addr = 0 ;
  n->state.oem_hi = OEM_HI;
  n->state.oem_lo = OEM_LO;
  n->state.esta_hi = ESTA_HI;
  n->state.esta_lo = ESTA_LO;
 ...
 * Binds to the network sockets and sends an ArtPoll
 *
...
int artnet_start(artnet_node vn) {
  ...
  // Die Verbindung zur Socket.DLL starten
...
#ifdef WIN32 // ### LH - port to win32
	// Die Verbindung zur Socket.DLL beenden
	WSACleanup();    
#endif  
...
int artnet_send_dmx(artnet_node vn, int port_id, int16_t length, uint8_t *data) {
  ...
  if (n->state.bcast_limit == 0) {
    ...
    int nodes = find_nodes_from_uni(&n->node_list, port->port_addr, ips, n->state.bcast_limit);
...
// need address here
/*int artnet_send_ipprog_reply(artnet_node vn) {
  node n = (node) vn;
  artnet_packet_t ipr;
...
int artnet_nl_update(node_list_t *nl, artnet_packet reply) {
  node_entry_private_t *entry;
	...
	// ### LH - 22.08.2008
	// Theoretically there could be more than one node
	// per IP (thats why i build in the option SO_REUSE...
	// find_entry_from_ip() only shows ONE node per IP
	// -> Allow several nodes per IP with different descriptions
	//
  //entry = find_entry_from_ip(nl, reply->from);
	entry = find_entry_from_ip_and_shortname(nl, reply);
...
/* ### LH - 22.08.2008
 * check if this packet is in list
 * node is unique with ip and shortname
 * thus it is possible to show more than one node per ip address
 * imagine DMX-Software (anrtnet-sender, artnet-receiver and artnet-monitor
 *  running on the same computer)
 */
node_entry_private_t *find_entry_from_ip_and_shortname(node_list_t *nl, artnet_packet reply)
{
  node_entry_private_t *tmp ;

  for (tmp = nl->first; tmp; tmp = tmp->next)
	{
    if ((reply->from.s_addr == tmp->ip.s_addr) && 
		    (strncmp((char *)reply->data.ar.shortname, (char *)tmp->pub.shortname, 
				 ARTNET_SHORT_NAME_LENGTH) == 0))
      break ;
  }
  return tmp ;
}
