...
#else
    if ((p.to.s_addr = inet_addr(ip)) == INADDR_NONE) {
#endif      
...
int artnet_tx_poll_reply(node n, int response) {
  ...
  snprintf((char *) &reply.data.ar.nodereport,sizeof(reply.data.ar.nodereport) , "%04hx [%04i] libartnet" , n->state.report_code , n->state.ar_count);
...
int artnet_tx_tod_request(node n) {
  ...
  for(i=0; i < ARTNET_MAX_PORTS; i++) {
    if(n->ports.out[i].port_enabled == TRUE) {
...
int artnet_tx_tod_data(node n, int id) {
  ...
  // this is interesting, the spec mentions TOD_ADD and TOD_SUBTRACT, but the codes aren't given
  // the windows drivers don't have these either....
 ...
 * @param action the action to perform. Either ARTNET_TOD_FULL or ARTNET_TOD_FLUSH
 */
...
int artnet_tx_firmware_packet(node n, firmware_transfer_t *firm ) {
  ...
  artnet_misc_int_to_bytes(firm->bytes_total / sizeof(uint16_t) , p.data.firmware.length);

  memcpy(&p.data.firmware.data, firm->data + (firm->bytes_current / sizeof(uint16_t) ), data_len);
...
//  if(n->state
  ...
  for(i= ARTNET_MAX_PORTS; i > 0; i--) {
    if( n->ports.out[i-1].port_enabled == TRUE  ||
      n->ports.in[i-1].port_enabled == TRUE )
      break;
  }
