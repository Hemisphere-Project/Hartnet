...
#define SHORT_NAME 	"ArtNet Node"
#define LONG_NAME 	"libartnet setdmx example"
...
void parse_args(opts_t *ops, int argc, char *argv[]) {
	...
	static struct option long_options[] = {
			...
			// ### LH - 09.10.2008
			{"broadcast", required_argument,	0, 's'},
			{"bind2IP", 	no_argument, 		0, 'b'},
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "a:p:c:d:f:vhs:brl:", long_options, &option_index);
...
int main(int argc, char *argv[]) {	
  ...
  if (!ops.bCast)
    ...
    strcpy(anp.cBcastAddr, ops.bCast) ;
  
  if ( (node = artnet_new_mod(ops.ip_addr, anp) ) == NULL )
	{
		printf("new failed %s\n" , artnet_strerror() ) ;
		goto error ;
	}		
  ...
  // node = artnet_new(ops.ip_addr, ops.verbose) ; ;
	...
	artnet_set_subnet_addr(node, 0) ; // LH
