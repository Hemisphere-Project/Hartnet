...
#ifndef WIN32 // ### LH - example not ready for win32 due to threading
...
//	int len ;
	...
	while(1) {
		...
		if(args->ops->fd[*pid] == UNCONNECTED) {
			...
			memcpy(buf, args->ops->dmx[*pid], ARTNET_MAX_DMX+1) ;
			pthread_mutex_unlock(&mem_mutex) ;

			do_write(args->ops, *pid, buf, ARTNET_MAX_DMX+1) ;
	//		sleep(1) ;
		}
	}
...
int main(int argc, char *argv[])
{	
	printf("Sorry: artnet-usb does not work on win32 so far\n") ;
